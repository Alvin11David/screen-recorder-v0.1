...
public class GoogleDriveService {
    ...
    public void renameFile(User user, String fileId, String newName) {
        if (fileId == null || fileId.isBlank()) return;
        ...
        try {
            ...
            HttpResponse<String> response = httpClient.send(request, HttpResponse.BodyHandlers.ofString());
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                log.warn("Failed to rename Drive file {}: HTTP {} body={}", fileId, response.statusCode(), response.body());
                throw new IllegalStateException("Failed to rename file in Google Drive");
            }
            log.info("[drive] renamed file {} to \"{}\" for user={}", fileId, newName, user.getEmail());
        } catch (IllegalArgumentException e) {
            throw e;
        } catch (Exception e) {
            log.warn("Failed to rename Drive file {}", fileId, e);
            throw new IllegalStateException("Failed to rename file in Google Drive");
        }
    }
